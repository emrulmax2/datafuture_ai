    ...
    public function handle()
    {
        ...
        if($notes->count() > 0):
            foreach($notes as $note):
                $employeeName = (isset($note->employee->full_name) && !empty($note->employee->full_name) ? $note->employee->full_name : 'N/A');
                $reminderSetBy = (
                    isset($note->user->employee->full_name) && !empty($note->user->employee->full_name)
                        ? $note->user->employee->full_name
                        : $note->user->name
                );
                $reminderSetByText = $reminderSetBy.(!empty($note->created_at) ? ' on '.date('jS F, Y \a\t h:i A', strtotime($note->created_at)) : '');
                ...
                $content .= '<!DOCTYPE html>
                    ...
                    <head>
                    <meta charset="UTF-8">
                    <title>Reminder Note Review Required</title>
                    </head>
                    ...
                    <strong>Action Required:</strong> Please review the reminder note below.
                    </p>
                    </td>
                    </tr>
                    ...
                    <td style="padding:34px 38px 28px 38px; font-size:15px; line-height:1.7; color:#374151; text-align:justify;">
                                ...
                                <p style="margin-top:0; text-align:left;">
                                    Dear <strong>HR Staff</strong>,
                    </p>
                    
                                <p style="text-align:justify;">
                                    Please check the note details and complete any required follow-up in the HR system.
                    </p>
                    ...
                    <td colspan="2" style="background-color:#f8fafc; padding:14px 18px; font-size:15px; font-weight:700; color:#1f2937; border-bottom:1px solid #e5e7eb;">
                                    ...
                                    <tr>
                    <th style="width:34%; text-align:left; vertical-align:top; padding:13px 18px; background-color:#f8fafc; color:#334155; font-size:14px; border-bottom:1px solid #e5e7eb;">
                                        Note
                    </th>
                    ...
                    <td style="padding:13px 18px; color:#374151; font-size:14px; border-bottom:1px solid #e5e7eb;">
                                    ...
                                    <tr>
                    <th style="width:34%; text-align:left; vertical-align:top; padding:13px 18px; background-color:#f8fafc; color:#334155; font-size:14px;">
                                        Reminder Set By
                    </th>
                    ...
                    <td style="padding:18px 20px; color:#334155; font-size:14px; line-height:1.7; text-align:justify;">
                                        Once completed, please close the reminder note to keep the record up to date.
                    </td>
                    </tr>
                    </table>
                ...
                UserMailerJob::dispatch($configuration, ['hr@lcc.ac.uk'], new CommunicationSendMail($subject, $content, [], false));
            endforeach;
        endif;
