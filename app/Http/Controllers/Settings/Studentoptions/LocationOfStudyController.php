...
use App\Exports\LocationOfStudyExport;
use App\Http\Controllers\Controller;
use App\Http\Requests\OptionValueRequest;
use App\Imports\LocationOfStudyImport;
use App\Models\LocationOfStudy;
use Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class LocationOfStudyController extends Controller
{
    public function list(Request $request){
        ...
        $query = LocationOfStudy::orderByRaw(implode(',', $sorts));
        if(!empty($queryStr)):
    ...
    public function update(OptionValueRequest $request){      
        $data = LocationOfStudy::where('id', $request->id)->update([
            'name'=> $request->name,
            'is_hesa'=> (isset($request->is_hesa) ? $request->is_hesa : 0),
            'hesa_code'=> (isset($request->is_hesa) && $request->is_hesa == 1 && !empty($request->hesa_code) ? $request->hesa_code : null),
            'is_df'=> (isset($request->is_df) ? $request->is_df : 0),
    ...
    public function destroy($id){
        $data = LocationOfStudy::find($id)->delete();
        return response()->json($data);
    }

    public function restore($id) {
        $data = LocationOfStudy::where('id', $id)->withTrashed()->restore();

        response()->json($data);
    }

    public function updateStatus($id){
        $title = LocationOfStudy::find($id);
        $active = (isset($title->active) && $title->active == 1 ? 0 : 1);

        LocationOfStudy::where('id', $id)->update([
            'active'=> $active,
            'updated_by' => auth()->user()->id
        ]);
    ...
    {
        return Excel::download(new LocationOfStudyExport(), 'location-of-study.csv');        
    }

    public function import(Request $request) {
        $file = $request->file('file');
        
        Excel::import(new LocationOfStudyImport(),$file);
