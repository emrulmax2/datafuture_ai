...
use App\Exports\ReasonForEndingCourseSessionExport;
use App\Http\Controllers\Controller;
use App\Http\Requests\OptionValueRequest;
use App\Imports\ReasonForEndingCourseSessionImport;
use App\Models\ReasonForEndingCourseSession;
use Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class ReasonForEndingCourseSessionController extends Controller
{
    public function list(Request $request){
        ...
        $query = ReasonForEndingCourseSession::orderByRaw(implode(',', $sorts));
        if(!empty($queryStr)):
    ...
    public function store(OptionValueRequest $request){
        $data = ReasonForEndingCourseSession::create([
            'name'=> $request->name,
    ...
    public function edit($id){
        $data = ReasonForEndingCourseSession::find($id);
    ...
    public function update(OptionValueRequest $request){      
        $data = ReasonForEndingCourseSession::where('id', $request->id)->update([
            'name'=> $request->name,
            'is_hesa'=> (isset($request->is_hesa) ? $request->is_hesa : 0),
            'hesa_code'=> (isset($request->is_hesa) && $request->is_hesa == 1 && !empty($request->hesa_code) ? $request->hesa_code : null),
            'is_df'=> (isset($request->is_df) ? $request->is_df : 0),
    ...
    public function destroy($id){
        $data = ReasonForEndingCourseSession::find($id)->delete();
        return response()->json($data);
    }

    public function restore($id){
        $data = ReasonForEndingCourseSession::where('id', $id)->withTrashed()->restore();

        return response()->json($data);
    }

    public function updateStatus($id){
        $title = ReasonForEndingCourseSession::find($id);
        $active = (isset($title->active) && $title->active == 1 ? 0 : 1);

        ReasonForEndingCourseSession::where('id', $id)->update([
            'active' => $active,
            'updated_by' => auth()->user()->id
        ]);
    ...
    public function export(){
        return Excel::download(new ReasonForEndingCourseSessionExport(), 'rsendcrss.csv');
    }
    
    public function import(Request $request) {
        $file = $request->file('file');
        
        Excel::import(new ReasonForEndingCourseSessionImport(),$file);
