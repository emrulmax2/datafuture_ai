...
use App\Exports\StudentSupportEligibilityExport;
use App\Http\Controllers\Controller;
use App\Http\Requests\OptionValueRequest;
use App\Imports\StudentSupportEligibilityImport;
use App\Models\StudentSupportEligibility;
use Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class StudentSupportEligibilityController extends Controller
{
    public function list(Request $request){
        ...
        $query = StudentSupportEligibility::orderByRaw(implode(',', $sorts));
        if(!empty($queryStr)):
    ...
    public function store(OptionValueRequest $request){
        $data = StudentSupportEligibility::create([
            'name'=> $request->name,
    ...
    public function edit($id){
        $data = StudentSupportEligibility::find($id);
    ...
    public function update(OptionValueRequest $request){      
        $data = StudentSupportEligibility::where('id', $request->id)->update([
            'name'=> $request->name,
            'is_hesa'=> (isset($request->is_hesa) ? $request->is_hesa : 0),
            'hesa_code'=> (isset($request->is_hesa) && $request->is_hesa == 1 && !empty($request->hesa_code) ? $request->hesa_code : null),
            'is_df'=> (isset($request->is_df) ? $request->is_df : 0),
    ...
    public function destroy($id){
        $data = StudentSupportEligibility::find($id)->delete();
        return response()->json($data);
    }

    public function restore($id) {
        $data = StudentSupportEligibility::where('id', $id)->withTrashed()->restore();

        response()->json($data);
    }

    public function updateStatus($id){
        $title = StudentSupportEligibility::find($id);
        $active = (isset($title->active) && $title->active == 1 ? 0 : 1);

        StudentSupportEligibility::where('id', $id)->update([
            'active'=> $active,
            'updated_by' => auth()->user()->id
        ]);
    ...
    {
        return Excel::download(new StudentSupportEligibilityExport(), 'student-support-eligibility.csv');        
    }

    public function import(Request $request) {
        $file = $request->file('file');
        
        Excel::import(new StudentSupportEligibilityImport(),$file);
