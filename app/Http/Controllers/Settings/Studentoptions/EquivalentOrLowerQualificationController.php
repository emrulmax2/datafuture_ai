...
use App\Imports\EquivalentOrLowerQualificationImport;
use App\Exports\EquivalentOrLowerQualificationExport;


class EquivalentOrLowerQualificationController extends Controller
{
    public function list(Request $request){
        ...
        $query = EquivalentOrLowerQualification::orderByRaw(implode(',', $sorts));
        if(!empty($queryStr)):
    ...
    public function store(OptionValueRequest $request){
        $data = EquivalentOrLowerQualification::create([
            'name'=> $request->name,
    ...
    public function edit($id){
        $data = EquivalentOrLowerQualification::find($id);
    ...
    public function update(OptionValueRequest $request){      
        $data = EquivalentOrLowerQualification::where('id', $request->id)->update([
            'name'=> $request->name,
            'is_hesa'=> (isset($request->is_hesa) ? $request->is_hesa : 0),
            'hesa_code'=> (isset($request->is_hesa) && $request->is_hesa == 1 && !empty($request->hesa_code) ? $request->hesa_code : null),
            'is_df'=> (isset($request->is_df) ? $request->is_df : 0),
    ...
    public function destroy($id){
        $data = EquivalentOrLowerQualification::find($id)->delete();
        return response()->json($data);
    }

    public function restore($id) {
        $data = EquivalentOrLowerQualification::where('id', $id)->withTrashed()->restore();

        response()->json($data);
    }

    public function updateStatus($id){
        $title = EquivalentOrLowerQualification::find($id);
        $active = (isset($title->active) && $title->active == 1 ? 0 : 1);

        EquivalentOrLowerQualification::where('id', $id)->update([
            'active'=> $active,
            'updated_by' => auth()->user()->id
        ]);
    ...
    {
        return Excel::download(new EquivalentOrLowerQualificationExport(), 'eqvorlwqf.csv');        
    }

    public function import(Request $request) {
        $file = $request->file('file');
        
        Excel::import(new EquivalentOrLowerQualificationImport(),$file);
