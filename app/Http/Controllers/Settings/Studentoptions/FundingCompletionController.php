...
use App\Exports\FundingCompletionExport;
use App\Http\Controllers\Controller;
use App\Http\Requests\OptionValueRequest;
use App\Imports\FundingCompletionImport;
use App\Models\FundingCompletion;
use Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class FundingCompletionController extends Controller
{
    public function list(Request $request){
        ...
        $query = FundingCompletion::orderByRaw(implode(',', $sorts));
        if(!empty($queryStr)):
    ...
    public function edit($id){
        $data = FundingCompletion::find($id);
    ...
    public function update(OptionValueRequest $request){      
        $data = FundingCompletion::where('id', $request->id)->update([
            'name'=> $request->name,
            'is_hesa'=> (isset($request->is_hesa) ? $request->is_hesa : 0),
            'hesa_code'=> (isset($request->is_hesa) && $request->is_hesa == 1 && !empty($request->hesa_code) ? $request->hesa_code : null),
            'is_df'=> (isset($request->is_df) ? $request->is_df : 0),
    ...
    public function destroy($id){
        $data = FundingCompletion::find($id)->delete();
        return response()->json($data);
    }

    public function restore($id) {
        $data = FundingCompletion::where('id', $id)->withTrashed()->restore();

        response()->json($data);
    }

    public function updateStatus($id){
        $title = FundingCompletion::find($id);
        $active = (isset($title->active) && $title->active == 1 ? 0 : 1);

        FundingCompletion::where('id', $id)->update([
            'active'=> $active,
            'updated_by' => auth()->user()->id
        ]);
    ...
    {
        return Excel::download(new FundingCompletionExport(), 'funding-completion.csv');        
    }

    public function import(Request $request) {
        $file = $request->file('file');
        
        Excel::import(new FundingCompletionImport(),$file);
