...
use App\Exports\ModuleResultExport;
use App\Http\Controllers\Controller;
use App\Http\Requests\OptionValueRequest;
use App\Imports\ModuleResultImport;
...
class ModuleResultController extends Controller
{
    public function list(Request $request){
        ...
        $query = ModuleResult::orderByRaw(implode(',', $sorts));
        if(!empty($queryStr)):
    ...
    public function update(OptionValueRequest $request){      
        $data = ModuleResult::where('id', $request->id)->update([
            'name'=> $request->name,
            'is_hesa'=> (isset($request->is_hesa) ? $request->is_hesa : 0),
            'hesa_code'=> (isset($request->is_hesa) && $request->is_hesa == 1 && !empty($request->hesa_code) ? $request->hesa_code : null),
            'is_df'=> (isset($request->is_df) ? $request->is_df : 0),
    ...
    public function destroy($id){
        $data = ModuleResult::find($id)->delete();
        return response()->json($data);
    }

    public function restore($id) {
        $data = ModuleResult::where('id', $id)->withTrashed()->restore();

        response()->json($data);
    }

    public function updateStatus($id){
        $title = ModuleResult::find($id);
        $active = (isset($title->active) && $title->active == 1 ? 0 : 1);

        ModuleResult::where('id', $id)->update([
            'active'=> $active,
            'updated_by' => auth()->user()->id
        ]);
    ...
    {
        return Excel::download(new ModuleResultExport(), 'funding-completion.csv');        
    }

    public function import(Request $request) {
        $file = $request->file('file');
        
        Excel::import(new ModuleResultImport(),$file);
