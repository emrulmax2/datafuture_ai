...
use App\Exports\HesaQualificationSubjectExport;
use App\Http\Controllers\Controller;
use App\Http\Requests\OptionValueRequest;
use App\Models\HesaQualificationSubject;
use Illuminate\Http\Request;
use App\Imports\HesaQualificationSubjectImport;
use Maatwebsite\Excel\Facades\Excel;

class HesaQualificationSubjectController extends Controller
{
    public function list(Request $request){
        ...
        $query = HesaQualificationSubject::orderByRaw(implode(',', $sorts));
        if(!empty($queryStr)):
    ...
    public function store(OptionValueRequest $request){
        $data = HesaQualificationSubject::create([
            'name'=> $request->name,
    ...
    public function edit($id){
        $data = HesaQualificationSubject::find($id);
    ...
    public function update(OptionValueRequest $request){      
        $data = HesaQualificationSubject::where('id', $request->id)->update([
            'name'=> $request->name,
            'is_hesa'=> (isset($request->is_hesa) ? $request->is_hesa : 0),
            'hesa_code'=> (isset($request->is_hesa) && $request->is_hesa == 1 && !empty($request->hesa_code) ? $request->hesa_code : null),
            'is_df'=> (isset($request->is_df) ? $request->is_df : 0),
    ...
    public function destroy($id){
        $data = HesaQualificationSubject::find($id)->delete();
        return response()->json($data);
    }

    public function restore($id) {
        $data = HesaQualificationSubject::where('id', $id)->withTrashed()->restore();

        response()->json($data);
    }

    public function updateStatus($id){
        $title = HesaQualificationSubject::find($id);
        $active = (isset($title->active) && $title->active == 1 ? 0 : 1);

        HesaQualificationSubject::where('id', $id)->update([
            'active'=> $active,
            'updated_by' => auth()->user()->id
        ]);
    ...
    {
        return Excel::download(new HesaQualificationSubjectExport(), 'Hesa_Qualification_Subject.csv');        
    }

    public function import(Request $request) {
        $file = $request->file('file');
        
        Excel::import(new HesaQualificationSubjectImport(),$file);
