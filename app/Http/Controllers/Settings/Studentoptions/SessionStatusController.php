...
use App\Exports\SessionStatusExport;
use App\Http\Controllers\Controller;
use App\Http\Requests\SessionStatusRequest;
use App\Imports\SessionStatusImport;
...
class SessionStatusController extends Controller
{
    public function index()
    {
        return view('pages/title/index', [
            'title' => 'Session Status - London Churchill College',
            'breadcrumbs' => [
                ['label' => 'Session Status', 'href' => 'javascript:void(0);']
            ],
        ]);
    }
    ...
    public function list(Request $request){
        ...
        $query = SessionStatus::orderByRaw(implode(',', $sorts));
        if(!empty($queryStr)):
    ...
    public function store(SessionStatusRequest $request){
        $data = SessionStatus::create([
            'name'=> $request->name,
    ...
    public function update(SessionStatusRequest $request){      
        $data = SessionStatus::where('id', $request->id)->update([
            'name'=> $request->name,
            'is_hesa'=> (isset($request->is_hesa) ? $request->is_hesa : 0),
            'hesa_code'=> (isset($request->is_hesa) && $request->is_hesa == 1 && !empty($request->hesa_code) ? $request->hesa_code : null),
            'is_df'=> (isset($request->is_df) ? $request->is_df : 0),
    ...
    public function destroy($id){
        $data = SessionStatus::find($id)->delete();
        return response()->json($data);
    }

    public function restore($id) {
        $data = SessionStatus::where('id', $id)->withTrashed()->restore();

        response()->json($data);
    }

    public function updateStatus($id){
        $title = SessionStatus::find($id);
        $active = (isset($title->active) && $title->active == 1 ? 0 : 1);

        SessionStatus::where('id', $id)->update([
            'active'=> $active,
            'updated_by' => auth()->user()->id
        ]);
    ...
    {
        return Excel::download(new SessionStatusExport(), 'SessionStatus.csv');        
    }

    public function import(Request $request) {
        $file = $request->file('file');
        
        Excel::import(new SessionStatusImport(),$file);
