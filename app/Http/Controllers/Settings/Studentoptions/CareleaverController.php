...
use App\Exports\CareLeaverExport;
use App\Http\Controllers\Controller;
use App\Http\Requests\CareLeaverRequest;
use App\Http\Requests\DisabilityRequest;
use App\Imports\CareLeaverImport;
use App\Models\CareLeaver;
use Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class CareleaverController extends Controller
{
    public function index()
    {
        return view('pages.settings.studentoption.careLeaver.index', [
            'title' => 'Care Leaver - London Churchill College',
            'breadcrumbs' => [
                ['label' => 'Care Leaver', 'href' => 'javascript:void(0);']
            ],
        ]);
    }
    ...
    public function list(Request $request){
        ...
        $query = CareLeaver::orderByRaw(implode(',', $sorts));
        if(!empty($queryStr)):
    ...
    public function store(CareLeaverRequest $request){
        $data = CareLeaver::create([
            'name'=> $request->name,
    ...
    public function update(DisabilityRequest $request){      
        $data = CareLeaver::where('id', $request->id)->update([
            'name'=> $request->name,
            'is_hesa'=> (isset($request->is_hesa) ? $request->is_hesa : 0),
            'hesa_code'=> (isset($request->is_hesa) && $request->is_hesa == 1 && !empty($request->hesa_code) ? $request->hesa_code : null),
            'is_df'=> (isset($request->is_df) ? $request->is_df : 0),
    ...
    public function destroy($id){
        $data = CareLeaver::find($id)->delete();
        return response()->json($data);
    }

    public function restore($id) {
        $data = CareLeaver::where('id', $id)->withTrashed()->restore();

        response()->json($data);
    }

    public function updateStatus($id){
        $title = CareLeaver::find($id);
        $active = (isset($title->active) && $title->active == 1 ? 0 : 1);

        CareLeaver::where('id', $id)->update([
            'active'=> $active,
            'updated_by' => auth()->user()->id
        ]);
    ...
    {
        return Excel::download(new CareLeaverExport(), 'careleaver.csv');        
    }

    public function import(Request $request) {
        $file = $request->file('file');
        
        Excel::import(new CareLeaverImport(),$file);
